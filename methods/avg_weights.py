...
def test_ptm_list(test_loader, net_list):
    ...
    return max_acc, acc_list, avg_accuracy


def weight_avg(args, net_list):
    assert args.base_model in ['GIN', 'GCN', 'GAT']

    if args.base_model == 'GIN':
        fusion_model = GIN_dict[args.model_arch](args)
    elif args.base_model == 'GCN':
        fusion_model = GCN_dict[args.model_arch](args)
    elif args.base_model == 'GAT':
        fusion_model = GAT_dict[args.model_arch](args)
    else:
        raise ('Not supporting such model!')
    ...
    with torch.no_grad():
        for name, param in fusion_model.named_parameters():
            # 获取所有模型该参数的值
            avg_param = torch.mean(torch.stack([model.state_dict()[name] for model in net_list]), dim=0)
            # 将平均值赋值给新模型
            param.copy_(avg_param)

    if args.gpu >= 0:
        fusion_model = fusion_model.to(f'cuda:{args.gpu}')
...
def main(args):
    ...
    net_list, net_arch = load_ptm(args)

    fusion_model = weight_avg(args, net_list)

    test_b_ac, precision, recall, f1, cm = test(test_loader, fusion_model, cross_ent)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gcls", type=int, default=2,
                        help="number of graph classes:{'IMDBBINARY': 2, 'MUTAG': 2, 'COLLAB': 3, 'PTC': 2, 'PROTEINS': 2, 'REDDITBINARY': 2, 'REDDITMULTI5K': 5, 'NCI1': 1}")
    ...
    for args.test_index in [2]:
        ...
        with open('train outs.txt', 'a') as file:
            print(f'\n{args.dataset}: avg_weight_acc', file=file)
